...
module Monad_example = {
  ...
  open X.Let_syntax;
  let _mf = (a) : X.t(_) => [%bind_open
    {
      let x = a;
      return(x + 1);
    }
  ];
  let _mf' = (a, b, c) : X.t(_) => [%bind_open
    {
      let x = a
      and y = b
      and (u, v) = c;
  ...
  let _mif' = (a) : X.t(_) => [%map if (a) {true} else {false}];
};
  ...
  module X: {
    ...
    module Let_syntax: {
      ...
      module Let_syntax: {
        ...
        let map: (t('a), ~f: 'a => 'b) => t('b);
        let both: (t('a), t('b)) => t(('a, 'b));
        module Open_on_rhs: {let flag: t(int); let anon: t(int);};
      };
    };
  } = {
    type t('a) = 'a;
...
module Example_without_open = {
  let _ag = (a) : Applicative_example.X.t(_) => [%map.Applicative_example.X
    {
      let x = a;
      x + 1;
    }
...
module Example_with_mapn = {
  module Let_syntax = {
    ...
    module Let_syntax = {
      ...
      let map3 = (a, b, c, ~f) => map2(both(a, b), c, ~f=((a, b), c) => f(a, b, c));
      let map4 = (a, b, c, d, ~f) =>
        map2(both(a, b), both(c, d), ~f=((a, b), (c, d)) => f(a, b, c, d));
    };
  };
  let _x =
    Let_syntax.(
